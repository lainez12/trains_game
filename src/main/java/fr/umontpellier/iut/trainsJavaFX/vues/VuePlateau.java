...
public class VuePlateau extends Pane {
    ...
    public VuePlateau () {
        ...
        setBackground(javafx.scene.layout.Background.fill(Color.AQUAMARINE));
    }
    ...
    public void creerBindings() {
        ...
        construirePlateau();
        mapVille.fitWidthProperty().addListener((obs, oldVal, newVal) -> setMaxWidth(mapVille.fitWidthProperty().get()));
        mapVille.fitHeightProperty().addListener((obs, oldVal, newVal) -> setMaxHeight(mapVille.fitHeightProperty().get()));
    }

    public void definirScaleTransformation() {
        Region conteneurParent = (Region) getParent();
        double height = (
                plateau.getHauteurInitialePlateau() / plateau.getLargeurInitialePlateau()
        );
        mapVille.fitWidthProperty().addListener((obs, oldVal, newVal) -> definirScalefacteur(conteneurParent.getWidth()*DonneesGraphiques.plateauWidth, mapVille.fitWidthProperty().get()*height));
        mapVille.fitHeightProperty().addListener((obs, oldVal, newVal) -> definirScalefacteur(conteneurParent.getWidth()*DonneesGraphiques.plateauWidth, mapVille.fitWidthProperty().get()*height));
    }
    ...
    private void bindRedimensionEtCentragePlateau() {
        mapVille.fitWidthProperty().bind(((Region) getParent()).widthProperty().multiply(DonneesGraphiques.plateauWidth));
        mapVille.fitHeightProperty().bind(mapVille.fitWidthProperty().multiply(
                plateau.getHauteurInitialePlateau() / plateau.getLargeurInitialePlateau()
        ));
        // Pour maintenir le plateau au centre
        /*mapVille.layoutXProperty().bind(new DoubleBinding() {
            {
    ...
    private void ajouteRailATuile(Tuile t, Joueur j, Circle c) {
        c.setFill(new ImagePattern(new Image("/images/icons/rail_"+ j.getCouleur() +".png")));
        t.ajouterRail(j);
    }

    private void choixTuile(MouseEvent event) {
        //event.getPickResult().getIntersectedPoint();
        Group tuile  = (Group) event.getSource();
        String id = tuile.getId();
        GestionJeu.getJeu().uneTuileAEteChoisie(id);
        //((VueDuJeu) getScene().getRoot()).getJeu().uneTuileAEteChoisie(id);
